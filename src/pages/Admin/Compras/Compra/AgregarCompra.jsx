...
const AgregarCompra = () => {
	...
	const actualizarCantidadTalla = (index, cantidad) => {
		...
		setDetalleActual((prev) => {
			...
			const sumaTotal = tallasActualizadas.reduce(
				(acc, t) => acc + t.cantidad,
				0,
			);
	...
	const agregarDetalleCompra = () => {
		if (!detalleActual.idItemSeleccionado) {
			return showAlert("Selecciona un insumo o producto", {
				type: "warning",
				title: "Cuidado!",
				duration: 2000,
			});
		...
		if (detalleActual.cantidad <= 0) {
			return showAlert("La cantidad debe ser mayor que 0", {
				type: "warning",
				title: "Cuidado!",
				duration: 2000,
			});
		...
		if (detalleActual.precio <= 0) {
			return showAlert("El precio debe ser mayor que 0", {
				type: "warning",
				title: "Cuidado!",
				duration: 2000,
			});
		...
		if (yaExiste) {
			return showAlert(
				`Este ${detalleActual.tipo.toLowerCase()} ya fue agregado`,
				{
					type: "error",
					duration: 2000,
				},
		...
		if (detalleActual.tipo === "Insumo") {
			if (!detalleActual.unidadMedida) {
				return showAlert("Selecciona una unidad de medida", {
					type: "warning",
				});
			}

			const factorConversion =
				conversionesUnidadAMl[detalleActual.unidadMedida] || 1;
			const cantidadOriginal = Number(detalleActual.cantidad);
			const cantidadEnMl = cantidadOriginal * factorConversion;

			if (cantidadEnMl === 0) {
				return showAlert("La cantidad convertida no puede ser cero", {
					type: "warning",
				});
			}
			...
			nuevoDetalle.Precio_Unitario =
				Number(detalleActual.precio) / cantidadEnMl;
		} else {
			...
			if (detalleActual.tallas.length) {
				const totalTallas = detalleActual.tallas.reduce(
					(suma, t) => suma + t.cantidad,
					0,
				);
				if (totalTallas !== nuevoDetalle.Cantidad) {
					return showAlert(
						"La suma de tallas debe coincidir con la cantidad total",
						{
							type: "error",
						},
					);
	...
	const esFrasco =
		detalleActual.tipo === "Insumo" &&
		insumoSeleccionado?.Categoria === "Frasco";
	...
	const handleSubmit = async (e) => {
		...
		if (!listaDetalles.length) {
			return showAlert("Agrega al menos un producto o insumo", {
				type: "error",
			});
		}
						...
						value={
							formData.Id_Proveedores
								? {
										...
										label:
											proveedores.find(
												(p) => p.Id_Proveedores === formData.Id_Proveedores,
											)?.Nombre || "",
									}
								: null
						}
					...
					<div className="grid grid-cols-7 gap-4 items-end">
						...
						{detalleActual.tipo === "Insumo" && (
							<div className="col-span-1">
								...
								<Select
									...
									value={
										opcionesUnidad.find(
											(op) => op.value === detalleActual.unidadMedida,
										) || null
									}
									onChange={(opcion) =>
										setDetalleActual((prev) => ({
						...
						<div className="col-span-1">
							...
							<input
								...
								min="1"
								value={
									detalleActual.cantidad === 0 ? "" : detalleActual.cantidad
								}
					...
					<div className="md:col-span-2 mt-6">
						...
						<div className="overflow-auto">
							<table className="w-full border-collapse">
								...
								<tbody>
									{listaDetalles
										.filter((d) => d.Id_Productos)
										.map((d, i) => {
											const producto = productos.find(
												(p) => p.Id_Productos === d.Id_Productos,
											);
											return (
												<tr key={i} className="border-b">
													...
													<td className="p-2">
														{d.tallas
															...
															.map((t) => {
																const nombreTalla =
																	producto?.Detalles.tallas.find(
																		(x) =>
																			x.Id_Producto_Tallas ===
																			t.Id_Producto_Tallas,
																	)?.Nombre;
													...
													<td className="p-2">
														<Button
															className="red"
															onClick={() => eliminarDetalleCompra(i)}
														>
					...
					<div className="md:col-span-2 mt-6">
						...
						<div className="overflow-auto">
							<table className="w-full border-collapse">
								...
								<tbody>
									{listaDetalles
										.filter((d) => d.Id_Insumos)
										.map((d, i) => {
											const insumo = insumos.find(
												(x) => x.Id_Insumos === d.Id_Insumos,
											);
											return (
												<tr key={i} className="border-b">
													...
													<td className="p-2">
														$
														{Math.round(d.Precio_Unitario).toLocaleString(
															"es-CO",
														)}
													</td>
													<td className="p-2">
