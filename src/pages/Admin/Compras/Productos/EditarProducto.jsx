...
const EditarProducto = () => {
	...
	useEffect(() => {
		...
		const esNuevaCategoria =
			!tallasDisponibles.length && categoriaSeleccionada.Es_Ropa;
	...
	const removeImagenExistente = (id) => {
		setImagenesExistentes((prev) =>
			prev.filter((img) => img.Id_Imagenes !== id),
		);
	...
	const fetchDatos = useCallback(async () => {
		try {
			...
			if (producto.Categoria.Es_Ropa) {
				const categoria = catRes.data.find(
					(cat) => cat.Id_Categoria_Producto === producto.Id_Categoria_Producto,
				);
				const tallasIds = (producto.TallasSeleccionadas || []).map((t) =>
					parseInt(t.Id_Tallas),
				);
		...
		try {
			...
			for (const key in formData) {
				if (key === "Id_Insumos" && formData.Id_Categoria_Producto == 3) {
					...
					(key === "Precio_Venta" || key === "Precio_Compra") &&
	...
	return (
		<>
			...
			>
				...
				{categorias.find(
					(c) => c.Id_Categoria_Producto == formData.Id_Categoria_Producto,
				)?.Es_Ropa && (
				...
				<div className="p-7 bg-white shadow border-2 border-gray-200 rounded-lg md:col-span-2 m-7 mt-2">
						...
						<div className="flex flex-wrap gap-4">
							{imagenesExistentes.length > 0 ? (
								imagenesExistentes.map((img, idx) => (
									<div
										key={`existente-${idx}`}
										className="relative w-[200px] h-[200px] border rounded shadow"
									>
										<img
											src={img.URL}
											alt={`img-${idx}`}
					...
					<div className="mb-6 w-full">
						...
						<div className="flex flex-wrap gap-4">
							{imagenesNuevas.length > 0 ? (
								...
								<p className="text-gray-500 italic">
									No se han seleccionado nuevas imágenes
								</p>
							)}
						</div>
					</div>
					...
					<div className="relative inline-block mt-2">
						<Button
							...
							icon="fa-upload"
							disabled={
								imagenesExistentes.length + imagenesNuevas.length >= maxImagenes
							}
						>
