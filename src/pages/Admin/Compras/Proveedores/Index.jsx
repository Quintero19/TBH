...
const filteredData = useMemo(() => {
    ...
    return data.filter(item => {
      ...
      const tipoMatch = item.Tipo_Proveedor?.toLowerCase().includes(lowerSearch);
      ...
      if (lowerSearch === "1" || lowerSearch === "activo") {
        estadoMatch = item.Estado === true || item.Estado === 1 || item.Estado === "Activo";
      } else if (lowerSearch === "0" || lowerSearch === "inactivo") {
        estadoMatch = item.Estado === false || item.Estado === 0 || item.Estado === "Inactivo";
      }

      return idMatch || tipoMatch || nombreMatch || celularMatch || emailMatch || estadoMatch;
    });
  }, [data, searchTerm]);
...
const handleVerDetalles = async (proveedor) => {
  try {
    ...
    Swal.fire('Error', 'No se pudieron cargar los detalles del proveedor', 'error');
  }
};
