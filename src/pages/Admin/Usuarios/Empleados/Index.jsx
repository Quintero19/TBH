...
const Empleados = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    return data.filter((item) => {
      const documentoMatch = item.Documento?.toString().toLowerCase().includes(lowerSearch);
      const nombreMatch = item.Nombre?.toLowerCase().includes(lowerSearch);
      const celularMatch = item.Celular?.toString().toLowerCase().includes(lowerSearch);

      const estadoMatch =
        (lowerSearch === "activo" && (item.Estado === true || item.Estado === 1)) ||
        (lowerSearch === "inactivo" && (item.Estado === false || item.Estado === 0));
