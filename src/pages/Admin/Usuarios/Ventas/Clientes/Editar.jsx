...
const EditarCliente = () => {
    ...
    useEffect(() => {
        const cargarCliente = async () => {
            ...
            try {
                ...
                const errorMessage = error.response?.data?.message || "No se pudo cargar el cliente.";
    ...
    const validarFormulario = async () => {
        if (!formData.TipoDocumento || !formData.Documento || !formData.Nombre || !formData.Correo || !formData.Celular || !formData.Direccion || !formData.FechaNacimiento) {
        ...
        try {
            ...
            const existeDocumento = clientes.some(
                (c) => c.Documento?.toString() === formData.Documento.toString() &&
            ...
            const existeCorreo = clientes.some(
                (c) => c.Correo?.toLowerCase() === formData.Correo.toLowerCase() &&
                       c.Id_Cliente !== formData.Id_Cliente
            );
            if (existeCorreo) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const errorMessage = error.response?.data?.message || "Ocurrió un error al actualizar el cliente.";
            showAlert(errorMessage, {
                type: "error",
                title: "Error",
            });
