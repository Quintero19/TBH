...
export default function Usuario() {
  ...
  const handleDateClick = (day) => {
    ...
    const selected = new Date(currentDate.getFullYear(), currentDate.getMonth(), parseInt(day));
  ...
  const generateCalendarDays = () => {
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      const isPast = isCurrent && dayDate < today && !(dayDate.getDate() === today.getDate() && dayDate.getMonth() === today.getMonth() && dayDate.getFullYear() === today.getFullYear());
      days.push({ 
        day: i.toString(), 
        disabled: isPast,
      ...
      <div className="content" style={{ flex: 1, padding: "20px" }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", margin: "20px 0" }}>
          ...
          <h2 style={{ margin: 0 }}>
            ...
            {isCurrent && <span style={{ fontSize: "0.8em", color: "#4CAF50", marginLeft: "10px" }}>(Mes actual)</span>}
          </h2>
          <button 
            onClick={() => changeMonth(1)}
            style={{ 
        ...
        <div style={{ margin: "20px 0" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: "10px" }}>
            {calendarDays.map(({day, disabled, isToday, selectable}, index) => (
              <div 
                key={index} 
                ...
                style={{
                  ...
                  backgroundColor: disabled ? "#f9f9f9" : 
                                    selectedDate === day ? "#4CAF50" : 
                                    selectable ? "#e3f2fd" : "white",
                  color: disabled ? "#ccc" : 
                         selectedDate === day ? "white" : 
                         isToday ? "#4CAF50" : 
                         selectable ? "#2196F3" : "black",
                  minHeight: "40px",
                  display: "flex",
                  alignItems: "center",
                ...
                title={!selectable ? "Solo se pueden agendar citas en el mes actual" : ""}
              >
                {day}
              </div>
            ))}
        ...
        {isCurrent && selectedDate && (
          <div>
            <h2>Selecciona una hora para el {selectedDate} de {monthNames[currentDate.getMonth()]}:</h2>
            <br />
                ...
                {timeSlots.morning.map((time) => (
                  <button
                    ...
                    style={{
                      ...
                      borderRadius: "4px",
                      cursor: "pointer",
                      backgroundColor: selectedTime === time ? "#2196F3" : "white",
        ...
        {!isCurrent && selectedDate && (
          <div style={{ 
            ...
            <h3 style={{ color: "#e65100", marginTop: 0 }}>Solo se pueden agendar citas en el mes actual</h3>
            <p>Por favor, navega al mes de {monthNames[now.getMonth()]} para agendar una cita.</p>
          </div>
        )}
      </div>
    </div>
